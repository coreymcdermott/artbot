...
class UNSWGalleriesSpider(CrawlSpider):
    ...
    def parse_exhibition(self, response):
        ...
        item['start']       = parser.parse(response.xpath('.//span[contains(@class, "date-display-start")]/@content').extract_first(), ignoretz=True)
        item['end']         = parser.parse(response.xpath('.//span[contains(@class, "date-display-end")]/@content').extract_first(), ignoretz=True)
